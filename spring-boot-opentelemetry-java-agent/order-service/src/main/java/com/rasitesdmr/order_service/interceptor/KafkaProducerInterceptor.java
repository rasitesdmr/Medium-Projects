package com.rasitesdmr.order_service.interceptor;

import com.rasitesdmr.order_service.domain.constant.Constants;
...
public class KafkaProducerInterceptor implements ProducerInterceptor<String, Object> {
    ...
    private void setKafkaClientName(ProducerRecord<String, Object> record) {
        if (record.headers().lastHeader(Constants.KAFKA_BASE_CLIENT_NAME) == null) {
            record.headers().add(Constants.KAFKA_BASE_CLIENT_NAME, Constants.KAFKA_CLIENT_ORDER_NAME.getBytes(StandardCharsets.UTF_8));
        }
    }
